 ...
 * @constructor
 */
const TorControl = function TorControl(opts) {
...
TorControl.prototype = {
  ...
  sendCommand: function sendCommandToTorCOntrol(command, cb, keepConnection) {
    ...
    return this.connect(null, (err, connection) => {
      if (err) {
        return cb(err);
      }
      connection.once('data', (data) => tryDisconnect(() => {
        let messages = [],
          arr,
          i;
        if (cb) {
  ...
  // GetInfo
  getInfo(request, cb) { // Chapter 3.9
  ...
  // Circuit
  extendCircuit(id, superspec, purpose, cb) { // Chapter 3.10
  ...
  setCircuitPurpose(id, purpose, cb) { // Chapter 3.11
    return this.sendCommand(`SETCIRCUITPURPOSE ${id} purpose=${purpose}`, cb);
  },


  setRouterPurpose(nicknameOrKey, purpose, cb) { // Chapter 3.12
    return this.sendCommand(`SETROUTERPURPOSE ${nicknameOrKey} ${purpose}`, cb);
  },
  attachStream(streamId, circuitId, hop, cb) { // Chapter 3.13
